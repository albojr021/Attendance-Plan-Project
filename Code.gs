...
return [];
  ...
  else if (sheetName === PLAN_SHEET_NAME) {
...
const planSheet = ss.getSheetByName(PLAN_SHEET_NAME);
        return !!ss.getSheetByName(empSheetName) && !!planSheet;
    } catch (e) {
...
const sheet = getOrCreateSignatoryMasterSheet(ss);
        ...
        // **MODIFIED: Read all columns to get Designation**
        const lastColumn = sheet.getLastColumn();
        const range = sheet.getRange(2, 1, sheet.getLastRow() - 1, lastColumn);
        const values = range.getDisplayValues();

        // **MODIFIED: Return objects with name and designation**
        return values.map(row => ({
            name: String(row[0] || '').trim(),
            designation: String(row[1] || '').trim() 
        })).filter(item => item.name); // Filter by name existence
} catch (e) {
...
// Use the passed group for a new plan (e.g., G2)
...
if (sheet) {
    ...
    try {
        ...
        // **MODIFIED: Added 'Designation' header**
        const headers = ['Signatory Name', 'Designation'];
        sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
        sheet.setFrozenRows(1);
        sheet.setColumnWidth(1, 200);
        sheet.setColumnWidth(2, 200); // Set width for new Designation column
        Logger.log(`[getOrCreateSignatoryMasterSheet] Created Signatory Master sheet: ${SIGNATORY_MASTER_SHEET} with Designation.`);
        return sheet;
} catch (e) {
...
function updateSignatoryMaster(signatories) {
    ...
    // 1. Collect all inputs from UI (Names and Designations)
    const inputList = [];
    // Approved By
    if (signatories.approvedBy) inputList.push({ 
        name: signatories.approvedBy.trim(), 
        designation: signatories.approvedByDesignation.trim() 
    });
    
    // Checked By
    signatories.checkedBy.forEach((name, i) => {
        if (name) inputList.push({ 
            name: name.trim(), 
            designation: signatories.checkedByDesignations[i].trim() 
        });
    });
    
    // Filter for unique names (case-insensitive check) and non-empty names
    const uniqueInputMap = new Map();
    inputList.forEach(item => {
        const key = item.name.toUpperCase();
        if (item.name) {
            // Keep the last entry if names are duplicated in the input
            uniqueInputMap.set(key, item);
        }
    });
    const uniqueInputList = Array.from(uniqueInputMap.values());

    if (uniqueInputList.length === 0) return;

    // 2. Get current data from sheet and map by name and sheet row index
    const lastRow = sheet.getLastRow();
    const headersCount = 2; // Name and Designation
    const existingDataRange = lastRow > 1 ? sheet.getRange(2, 1, lastRow - 1, headersCount) : null;
    const existingValues = existingDataRange ? existingDataRange.getValues() : [];

    const existingMap = new Map(); // Key: UPPERCASE Name, Value: {row: N, designation: D}

    existingValues.forEach((row, index) => {
        const name = String(row[0]).trim();
        const designation = String(row[1]).trim();
        if (name) {
             // +2 because array index starts at 0, and sheet data starts at row 2
            existingMap.set(name.toUpperCase(), { row: index + 2, designation: designation }); 
        }
    });

    const rowsToAppend = [];
    const updates = []; // Array of {row: N, designation: D}

    // 3. Process inputs against existing data
    uniqueInputList.forEach(input => {
        const key = input.name.toUpperCase();
        const existingRecord = existingMap.get(key);
        
        // Ensure that the user has provided a designation that meets the minimum length
        const inputDesignation = input.designation; 

        if (existingRecord) {
            // Check if designation changed (case-insensitive and if input is not empty)
            if (inputDesignation && existingRecord.designation.toUpperCase() !== inputDesignation.toUpperCase()) {
                updates.push({ row: existingRecord.row, designation: inputDesignation });
            }
        } else if (input.name) {
            // New name detected. Save the name and the designation provided by the user.
            rowsToAppend.push([input.name, inputDesignation]);
        }
    });

    // 4. Apply Updates (Designations for existing names)
    if (updates.length > 0) {
        updates.forEach(update => {
            // Update only the Designation column (Column 2)
            sheet.getRange(update.row, 2).setValue(update.designation);
        });
        Logger.log(`[updateSignatoryMaster] Updated designations for ${updates.length} existing signatories.`);
    }

    // 5. Append New Rows
    if (rowsToAppend.length > 0) {
        sheet.getRange(sheet.getLastRow() + 1, 1, rowsToAppend.length, headersCount).setValues(rowsToAppend);
        Logger.log(`[updateSignatoryMaster] Appended ${rowsToAppend.length} new signatories (Name and Designation).`);
    }
}
