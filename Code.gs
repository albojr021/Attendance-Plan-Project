...
// --- Function getSheetData() is UNCHANGED ---
...
// --- Function getContracts() is UNCHANGED ---
...
// --- NEW: Function to handle ALL saves in bulk ---
/**
 ...
 */
function saveAllData(contractId, employeeChanges, attendanceChanges) {
    if (!contractId) throw new Error("Contract ID is required.");
    ensureContractSheets(contractId);
    
    // 1. I-save ang Employee Info (Bulk)
    if (employeeChanges && employeeChanges.length > 0) {
        saveEmployeeInfoBulk(contractId, employeeChanges);
    }
    
    // 2. I-save ang Attendance Plan (Bulk)
    if (attendanceChanges && attendanceChanges.length > 0) {
        saveAttendancePlanBulk(contractId, attendanceChanges);
    }
    
    Logger.log(`[saveAllData] Successfully saved ${employeeChanges.length} employee updates and ${attendanceChanges.length} attendance updates for ${contractId}.`);
}


// --- REPLACING old single-row saveAttendancePlan ---
/**
...
function saveAttendancePlanBulk(contractId, changes) {
    ...
    planSheet.setFrozenRows(0); // Temporarily unfreeze to read/write all
    const range = planSheet.getDataRange();
    const values = range.getValues();
    const headers = values[0]; 
    planSheet.setFrozenRows(1); // Restore freeze
    ...
    const rowsToUpdate = {}; // Key: i+1 (sheet row number), Value: newStatus
    const newRows = [];
    
    changes.forEach(data => {
        let rowFound = false;
        // Hanapin ang existing row (magsimula sa values[1] = row 2 sa Sheet)
        for (let i = 1; i < values.length; i++) {
            const row = values[i];
            if (
                ...
                rowsToUpdate[i + 1] = data.status; // i+1 is the actual sheet row number
                rowFound = true;
                break;
            }
        }
...
// --- REPLACING old single-row saveEmployeeInfo ---
/**
...
function saveEmployeeInfoBulk(contractId, changes) {
    ...
    for (let i = 1; i < values.length; i++) {
        personnelIdMap[(values[i][personnelIdIndex] || '').trim()] = i + 1; // Store row number
    }
    
    changes.forEach(data => {
        const oldId = (data.oldPersonnelId || '').trim();
...
// --- Dati nang functions, inalis/pinalitan ng Bulk operations: ---
// function saveAttendancePlan(data) { ... } (REMOVED)
// function saveEmployeeInfo(data) { ... } (REMOVED)
